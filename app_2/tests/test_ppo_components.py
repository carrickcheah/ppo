"""
Comprehensive tests for PPO model components
"""
...
class TestStateEncoder:
    ...
    def test_encoding_from_observation(self):
        ...
        assert 'jobs' in encoded, "Missing 'jobs' in encoded state"
        assert 'machines' in encoded, "Missing 'machines' in encoded state"
        assert 'mask' in encoded, "Missing 'mask' in encoded state"
        
        # Check shapes
        assert encoded['jobs'].shape == (2, n_jobs, 128), f"Wrong job encoding shape: {encoded['jobs'].shape}"
        assert encoded['machines'].shape == (2, n_machines, 128), f"Wrong machine encoding shape: {encoded['machines'].shape}"
        assert encoded['mask'].shape == (2, n_jobs, n_machines), f"Wrong mask shape: {encoded['mask'].shape}"
        
        print(f"✓ State encoder produces correct shapes")
        print(f"  - Jobs: {encoded['jobs'].shape}")
        print(f"  - Machines: {encoded['machines'].shape}")
        print(f"  - Mask: {encoded['mask'].shape}")
...
class TestPPOScheduler:
    ...
    def test_model_initialization(self):
        ...
        # Test forward pass
        obs = np.random.randn(1, 150).astype(np.float32)  # 10 jobs * 10 + 5 machines * 5
        action_mask = np.ones(50, dtype=bool)  # 10 * 5
        
        action, value, log_prob = model.get_action(obs, action_mask, n_jobs=10, n_machines=5)
        ...
        print(f"✓ Model inference successful - action: {action}, value: {value:.3f}")
