"""
TRULY FIXED Curriculum Environment - NO FREE REWARDS
"""
...
class CurriculumEnvironmentTrulyFixed(gym.Env):
    """Environment that ONLY rewards actual scheduling actions."""
    ...
    def __init__(self,
        ...
        # Action space with no-action support
        ...
        # CRITICAL FIX: No free rewards at episode end!
        self.reward_config = reward_config or {
            'no_action_penalty': -1.0,       # Small penalty for no action
            'invalid_action_penalty': -10.0, # Large penalty for invalid
            'valid_schedule_reward': 10.0,   # Reward for scheduling
        ...
        if verbose:
            logger.info(f"TRULY FIXED Environment for {stage_name}:")
            logger.info(f"  NO final episode bonus - only action rewards!")
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        """Execute action - NO FREE REWARDS!"""
        ...
        info = {
            ...
            'action_type': 'unknown',
            'scheduled_job': None,
            'jobs_scheduled': len(self.scheduled_jobs),
        ...
        # NO FINAL REWARD! The agent must earn rewards through actions
    ...
    def _try_schedule(self, family_id: str, machine_id: int, info: Dict) -> float:
        ...
        reward = self.reward_config['valid_schedule_reward']
        
        # Add sequence completion bonus
        reward += self.reward_config['sequence_completion']
        
        # Family completion bonus
        if progress['completed_sequences'] >= progress['total_sequences']:
            self.completed_jobs.add(family_id)
            reward += self.reward_config['family_completion']
        ...
        if days_until_lcd >= 0:
            ...
            reward += self.reward_config['late_penalty_per_day'] * days_until_lcd
        
        info['action_valid'] = True
        info['action_type'] = 'schedule'
        info['scheduled_job'] = job_key
        info['reward_breakdown'] = {
            'base': self.reward_config['valid_schedule_reward'],
            'sequence': self.reward_config['sequence_completion'],
            'family': self.reward_config['family_completion'] if family_id in self.completed_jobs else 0,
            'deadline': self.reward_config['on_time_bonus'] if days_until_lcd >= 0 else days_until_lcd * self.reward_config['late_penalty_per_day']
        }
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        for fid in self.family_ids:
            ...
            obs.extend([
                ...
                progress['next_sequence'] / progress['total_sequences'],
                float(fid in self.completed_jobs)
            ])
        ...
        for mid in self.machine_ids:
            ...
            obs.extend([
                ...
                float(len(self.machine_schedules[mid]))
            ])
        ...
        obs.extend([
            ...
            float(len(self._get_valid_actions()) > 1)  # Has valid actions?
        ])
